...
import FaveIcon from "./faveIcon.svelte";
    ...
    //export let cls = "fave-icon";
    let name = 'star-full';
    let isLiked = false;
    $: {
        if($userInfoStore.isLogin) {
            isLiked = $userInfoStore.me.likes.includes(product_id);
            if(isLiked){
                name='star-full';
            }else {
                name='star-full';
...
<Button on:click={like_clicked} class="fave-icon">
    <svg class="icon" width="2.5rem" height="2.5rem" viewBox="0 0 {256} {256}">
        <g transform="translate(128 128) scale(0.72 0.72)" style="">
            <g style="stroke: none; stroke-width: 0; stroke-dasharray: none; stroke-linecap: butt; stroke-linejoin: miter; stroke-miterlimit: 10; fill: none; fill-rule: nonzero; opacity: 1;" transform="translate(-175.05 -175.05000000000004) scale(3.89 3.89)" >
