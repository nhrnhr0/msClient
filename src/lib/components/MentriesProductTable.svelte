...
function input_amount_changed(e) {
  ...
  if (amount == 0 || amount == undefined) {
    ...
    requestAnimationFrame(() => {
      fly_to_cart(document.querySelector(".top-info .product-image img"));
    });
                    ...
                    {#each productInfo.varients as { id, name }, idx}
                      ...
                      <div class="cell-wraper">
                        <input
                          ...
                          on:change={input_amount_changed}
                          style="border: 2px solid {ALL_COLORS[color].color}"
                          id="input_entery_{productInfo.id}_{size}_{color}_{id}"
                          class="size-input cls-cell"
                          type="number"
                          placeholder={name}
      ...
      <div class="show_sizes_popup_false">
        ...
        {#if productInfo.colors.length > 1}
          <div class="disclaimer">
            <h5>
              חבילה יכולה להכיל את הצבעים הבאים:
              <div class="flex-list">
        ...
        {#if productInfo.varients.length > 1}
          <div class="disclaimer">
            <h5>
              חבילה יכולה להכיל את המודלים הבאים:
              <div class="flex-list">
                {#each productInfo.varients as varient}
                  <div class="item">
        ...
        {#if productInfo.sizes.length > 1}
          <div class="disclaimer">
            <h5>
              חבילה יכולה להכיל את המידות הבאות:
              <div class="flex-list">
                {#each productInfo.sizes as size_id}
                  <div class="item">
