...
<div class="product-page-wraper" class:loading={!done_loading}>
  <div class="product-show-wrapper-grid">
    <div
      class="top-info"
      bind:clientWidth={top_info_w}
      class:flex-col={top_info_w < 700}
    >
      <div class="product-image" class:can-make-big={done_loading}>
        ...
        {#if productInfo}
          ...
          <!--{#if $cartStore[slimData.id]}
                        ...
                        {/if}-->
          <img
            src={CLOUDINARY_URL + productInfo?.cimage}
            alt={productInfo?.title}
            on:click={() => {
              big_image_selected = true;
            }}
          />
          <PriceTag
        ...
        <div class="info-wraper">
          {#if productInfo}
            <div class="product-title">
              <h2>
                {productInfo?.title}
                {#if $userInfoStore.isLogin && $userInfoStore.me.is_superuser && $dictCartStore[productInfo.id] && $userInfoStore.me.show_prices == true}
                  <span
                    class="price-edit"
    ...
    <div class="product-mentries">
      {#if productInfo}
        {#key productInfo.id}
          <MentriesProductTable {productInfo} />
        {/key}
      {:else}
  ...
  .similer-products-list {
    ...
    & .product-item {
      ...
      &:hover {
        ...
        background: radial-gradient(
          circle,
          rgba(167, 166, 166, 0.199) 0%,
          rgba(177, 175, 175, 0.199) 100%
        );
      }
      a {
        color: black;
