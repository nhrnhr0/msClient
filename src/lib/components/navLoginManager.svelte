  ...
  <div class="login-btn-wraper">
    <div class="text">
      {#if !$userInfoStore.isLogin}
        התחבר
      {:else}
        {$userInfoStore.me.username.length > 7 ? "..." + $userInfoStore.me.username.substring(0, 6) : $userInfoStore.me.username}
      {/if}
    </div>
    <button class="btn login-btn" on:click={login_btn_click}>
      {#if !$userInfoStore.isLogin}
        <svg class="popup-animation" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="32px" height="32px" fill="var(--text-on-background-color)">
          <path
