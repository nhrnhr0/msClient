...
<div class="position-wraper">
  <div
    class="acordion-button"
    class:close={is_side_closed}
    on:click={() => {
      is_side_closed = !is_side_closed;
    }}
  >
    <div class="text">
      {#if is_side_closed}
  ...
  {#if is_side_closed == false}
    <div
      class="background-wraper"
      in:fly={{ x: 200, duration: 350 }}
      out:fly={{ x: 200, duration: 350 }}
    >
      <h6>תתי קטגוריות</h6>
      <div class="album-swiper-wraper" class:loading>
        <div bind:this={slider} class="scroll-snap-slider">
          <!-- on:mousedown="{slider_mousedown}" on:mouseleave="{slider_mouseleave}" on:mouseup="{slider_mouseup}" on:mousemove={slider_mousemove}  -->
          ...
          {#each my_albums || [] as category}
            <div
              on:click={slider_click}
              class="scroll-snap-slide"
              class:active={category.slug == $page.query.get("album")}
              data-album-slug={category.slug}
            >
              <div class="card">
                <img
                  width="25px"
                  height="25px"
                  src="{CLOUDINARY_URL}{category.cimage}"
                  alt={category.title}
                />
