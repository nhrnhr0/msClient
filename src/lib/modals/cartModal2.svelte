...
import { BASE_URL, CLOUDINARY_URL, STATIC_BASE, SUBMIT_CART_URL } from "./../../api/consts";
                        ...
                        <tr>
                          ...
                          {#if $userInfoStore?.me?.is_superuser}
                            <td class="hide-on-md">
                              <div
                                on:click={(e) => {
                                  $cartStore[key].print = !$cartStore[key].print;
                                }}
                              >
                            ...
                            <td class="hide-on-md">
                              <div
                                on:click={(e) => {
                                  $cartStore[key].embro = !$cartStore[key].embro;
                                }}
                              >
                    ...
                    <div class="totals">
                      ...
                      <div class="product-total-price-result">
                        {roundHalf(
                          Object.entries($cartStore).reduce((acc, [key, val]) => {
                            ...
                            return acc + (val.newPrice || val.price) * val.amount * (val.out_of_stock == false ? 1 : 0);
                          }, 0)
                        )}₪
                      </div>
                      ...
                      {roundHalf(
                        Object.entries($cartStore).reduce((acc, [key, val]) => {
                          ...
                          return acc + (val.newPrice || val.price) * val.amount * 1.17 * (val.out_of_stock == false ? 1 : 0);
                        }, 0)
                      )}₪
                    </div>
                  {/if}
                ...
                <div in:fly={{ x: -340 }} class="step step-2">
                  <div class="form-group">
                    ...
                    <div class="form-control">
                      ...
                      <input bind:value={form_email} name="email" placeholder={$userInfoStore?.actAs?.email || $userInfoStore?.me?.email} type="email" />
                    </div>

                    <div class="form-control">
                      <label for="phone">טלפון</label>
                      <input bind:value={form_phone} name="tel" required={!($userInfoStore && $userInfoStore.isLogin)} placeholder="טלפון" type="tel" />
                    </div>
                  ...
                  <div class="form-group">
                    <div class="form-control">
                      <textarea bind:value={form_message} name="message" required={false} placeholder="הודעה:" />
                    </div>
                  </div>
                  <div class="form-group">
                    <div class="form-control">
              ...
              <div class="send-wra">
                {#if currentStep == 1}
                  <button on:click={stepBtnClick} class="submit-btn btn">
                    הבא
                    <img class="arrow-left" width="32px" height="32px" alt="right arrow" src="/right-arrow-hover.png" />
                  </button>
                {:else}
                  <button on:click={stepBtnClick} class="submit-btn btn">
                    {#if isSending}
    ...
    <div class="sidebar-cart-wraper">
      <aside on:click|preventDefault={() => {}} transition:fly={{ x: 340 }} id="sidebar-cart" style:top={sidebar_top + "px"}>
        <main>
          ...
          >
          <h2>
            מוצרים שאהבתי<span class="count">{Object.keys($cartStore).length}</span>
          </h2>
                        ...
                        <div class="table">
                          ...
                          <button class="edit-btn">
                            ...
                            <svg
                              ...
                              ><g
                                ><rect height="23" transform="matrix(-0.7071 -0.7072 0.7072 -0.7071 38.2666 48.6029)" width="11" x="23.7" y="4.875" /><path
                                ...
                                /><polygon points="16,22.229 16,30 23.246,30  " /><path
          ...
          <div class="action-buttons">
            {#if state == 0}
              <button class="checkout-button" on:click={checkout_click} disabled={state == 2}> לקופה </button>
