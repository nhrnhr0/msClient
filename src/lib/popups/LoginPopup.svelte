...
function update_admin_as_user() {
  if (admin_as_user) {
    $userInfoStore.actAs = all_users.find((user) => {
      return user.id == parseInt(admin_as_user.id);
    });
          ...
          {#if $userInfoStore.isLogin}
            <div class="user-data">
              ...
              <div class="action-buttons">
                ...
                {#if $userInfoStore.me.is_superuser}
                  <button
                    on:click|preventDefault|stopPropagation={refresh_all_users}
                  >
                    {#if refreshing_users}
                      <Spinner />
                    {:else}
                  ...
                  <button
                    on:click|preventDefault|stopPropagation={update_admin_as_user}
                    >פעל בתור מתשמש</button
                  >
                  <button
                    on:click|preventDefault|stopPropagation={remove_admin_as_user}
                    >חזור לאדמין</button
                  >
                {/if}
              </div>
          ...
          {:else}
            <div class="login-form">
              ...
              <div class="form-group">
                ...
                <input
                  on:keypress={keypress_submit_on_enter}
                  type={password_type}
                  class="form-control"
                  id="password"
                  placeholder="סיסמה"
