<script>
import { send_product_photo } from "src/api/api";
...
let image_src = '';
let modal_zIndex = 100;
let is_sending = false;
let inputfile;
let mfile;
let description_bind;
function closeModal() {
    productPhotoPopupStore.close();
}
...
<div id="singleAmountModal" style="z-index: {modal_zIndex};" class="modal" class:active={$productPhotoPopupStore.isOpen}>
    <div class="overlay" style="z-index: {modal_zIndex+5};" on:click={closeModal}>
        {#if $productPhotoPopupStore.isOpen}
