    ...
    for (let i = 0; i < detail.length; i++) {
      ...
      if (target) {
        ...
        if (
          ...
          dont_open_modal == false
        ) {
          $productModalStore.setProduct(
            target.dataset.catalogId,
          ...
          target.classList.contains("read-more-btn") &&
          dont_open_modal == false
        ) {
          $productModalStore.setProduct(
            target.dataset.catalogId,
...
function background_click(e) {
  ...
  if (
    classs.contains("lazy-swiper-wraper") ||
    classs.contains("swiper") ||
    classs.contains("swiper-wrapper") ||
    classs.contains("swiper-pagination")
  ) {
    open_category_modal();
    return;
  }
...
function clickable_category_mouse_enter(e) {
  ...
  if (
    ...
    e.target.classList.contains("lazy-swiper-wraper") ||
...
const show_prices =
  $userInfoStore["me"] &&
  Object.keys($userInfoStore["me"]) != 0 &&
...
<div class="title-wraper" class:campain={album.is_campain}>
  <button
    class:active={active_hover}
    class="title btn"
    on:click={open_category_modal(album)}
  >
    {album.title}

    {#if album.is_campain}
      <MyCountdown
        date={$campainsStore.find((v) => v.album.id == album.id)?.endTime}
      />
    {/if}
  </button>
</div>
<div
  class="lazy-swiper-wraper"
  on:focus={clickable_category_mouse_enter}
  on:mouseover={clickable_category_mouse_enter}
  on:blur={clickable_category_mouse_enter}
  on:mouseleave={clickable_category_mouse_leave}
  on:click={(e) => {
    background_click(e);
  }}
  class:active={isInView}
    ...
    {:else}
      <Swiper
        ...
        on:realIndexChange={(event) => {
          ...
          let productId = mswiper
            .swiper()
            .slides[idx].querySelector(".product-image").dataset.productId;
      ...
      >
        {#each loaded_data as image, index (image.id)}
          <SwiperSlide bind:this={slides[index]} data-product-id={image.id}>
            ...
            <div
              class="img-title"
              class:active={$cartStore[image.id] != undefined}
            >
              <div class="content">
                {image.title}
              </div>
            ...
            <div class="slide-content">
              <div class="img-wraper">
                <div
                  class="delete-btn delete-btn-click-area"
                  class:show={$cartStore[image.id] != undefined}
                >
                  <svg
                    fill="#000000"
                ...
                <div class="question-wraper swiper-question-click-area">
                  <SvelteTooltip tip="שאל על המוצר" left>
                    <QuestionLabel
                      class="swiper-question swiper-question-click-area"
                      AllClassName={"swiper-question-click-area"}
                      product_id={image.id}
            ...
            <div class="like-btn like-btn-small">
              <button
                data-product-id={image.id}
                data-catalog-id={album.id}
                class:gold={$cartStore[image.id] != undefined}
              ...
              <button
                data-product-id={image.id}
                data-catalog-id={album.id}
                class="action-btn read-more-btn"
              >
                פרטים
              </button>
            </div>
      ...
      <button
        on:click={(e) => {
          mswiper.swiper().slideNext();
        }}
        class="nav-btn left"
        on:mouseenter={() => {
          mouse_over_left_arrow = true;
        }}
        on:mouseleave={() => {
          mouse_over_left_arrow = false;
        }}
      >
      ...
      <button
        on:click={(e) => {
          mswiper.swiper().slidePrev();
        }}
        class="nav-btn right"
        on:mouseenter={() => {
          mouse_over_right_arrow = true;
        }}
        on:mouseleave={() => {
          mouse_over_right_arrow = false;
        }}
      >
