...
import { ALBUMS_API_URL, SIZES_API_URL, COLORS_API_URL, LOGOS_API_URL, MAIN_PAGE_API } from "./../api/consts";

import { api_get_user_campains, fetch_wraper, get_products_info } from "./../api/api";
...
export async function load({ fetch, page }) {
  ...
  //MAIN_PAGE_API
  let response = await fetch_wraper(
    MAIN_PAGE_API,
    {
      method: "GET",
      //redirect: 'follow'
...
const meta_data = {
  ...
  image:
    "https://res.cloudinary.com/ms-global/image/upload/c_scale,w_219,h_219/c_scale,u_v1649744644:msAssets:image_5_qo7yhx.jpg,w_300/v1/bizbiz/z6bktayusj7uc7rl2xgg.png",
};
</script>
...
{#if $userInfoStore.isLogin == true && $userInfoStore.me && $userInfoStore.me.show_prices == true}
  <div class="tax-text">המחירים באתר אינם כוללים מע"מ</div>
{/if}
<!--
<FavoritesSidePopup />
-->
...
{#each albums as album, i (album.id)}
  <CatalogSwiper {album} bind:this={$all_swipers[album.id]} loaded_data={all_products[album.id]} />
